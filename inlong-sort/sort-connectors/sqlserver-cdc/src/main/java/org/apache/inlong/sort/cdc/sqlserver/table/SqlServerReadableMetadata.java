...
public enum SqlServerReadableMetadata {
    ...
    ORACLE_TYPE(
            ...
            new MetadataConverter() {
                ...
                public Object read(
                    ...
                    Map<String, String> oracleType = getSqlServerType(tableSchema);
                    if (oracleType == null) {
                        return null;
                    }
    ...
    private static StringData getCanalData(SourceRecord record, TableChange tableSchema, GenericRowData rowData) {
        ...
        CanalJson canalJson = CanalJson.builder()
                ...
                .sqlServerType(getSqlServerType(tableSchema))
    ...
    public static Map<String, String> getSqlServerType(@Nullable TableChanges.TableChange tableSchema) {
        if (tableSchema == null) {
            return null;
        }
        Map<String, String> sqlServerType = new LinkedHashMap<>();
        final Table table = tableSchema.getTable();
        for (Column column : table.columns()) {
            // The typeName contains precision and does not need to be formatted.
            if (column.typeName().matches(REGEX_FORMATTED)) {
                sqlServerType.put(column.name(), column.typeName());
                continue;
            }
