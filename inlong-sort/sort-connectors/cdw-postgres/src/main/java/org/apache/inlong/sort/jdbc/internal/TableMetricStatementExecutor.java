...
public final class TableMetricStatementExecutor implements JdbcBatchStatementExecutor<RowData> {
    ...
    public TableMetricStatementExecutor(StatementFactory stmtFactory, JdbcRowConverter converter,
            DirtySinkHelper<Object> dirtySinkHelper, SinkMetricData sinkMetricData, String tableIdentifier,
            JdbcSchemaSchangeHelper helper) {
    ...
    public void executeBatch() throws SQLException {
        try {
            ...
            if (multipleSink && helper.handleResourceNotExists(tableIdentifier, e)) {
                try {
                    doExecuteBatch();
                } catch (SQLException ex) {
                    tryHandleExecuteWhenError(ex);
                }
            } else {
                tryHandleExecuteWhenError(e);
            }
        }
    }

    private void tryHandleExecuteWhenError(SQLException e) throws SQLException {
    ...
    private void doExecuteBatch() throws SQLException {
        st.executeBatch();
