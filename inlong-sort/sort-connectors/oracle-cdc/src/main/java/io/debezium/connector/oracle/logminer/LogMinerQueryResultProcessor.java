...
import org.apache.inlong.sort.cdc.oracle.source.relational.OracleSchemaChangeEventEmitter;
...
class LogMinerQueryResultProcessor {
    ...
    private static DmlParser resolveParser(OracleConnectorConfig connectorConfig,
    ...
    void processResult(ResultSet resultSet) throws SQLException {
        ...
        while (context.isRunning() && hasNext(resultSet)) {
            ...
            String redoSql = RowMapper.getSqlRedo(resultSet, dml, historyRecorder, scn, tableName, segOwner,
                    operationCode, changeTime, txId);

            LOGGER.trace(
                    "scn={}, operationCode={}, operation={}, table={}, segOwner={}, userName={}, rowId={}, rollbackFlag={}",
                    scn, operationCode, operation,
            ...
            switch (operationCode) {
                ...
                case RowMapper.SELECT_LOB_LOCATOR: {
                    ...
                    if (table == null) {
                        LogMinerHelper.logWarn(streamingMetrics,
                                "SEL_LOB_LOCATOR for table '{}' is not known to the connector, skipped.", tableId);
                        continue;
                    }
                    transactionalBuffer.registerSelectLobOperation(operationCode, txId, scn, tableId,
                            changeTime.toInstant(),
                ...
                case RowMapper.LOB_WRITE: {
                    ...
                    if (schema.tableFor(tableId) == null) {
                        LogMinerHelper.logWarn(streamingMetrics,
                                "LOB_WRITE for table '{}' is not known to the connector, skipped.", tableId);
                        continue;
                    }
                ...
                case RowMapper.LOB_ERASE: {
                    ...
                    if (schema.tableFor(tableId) == null) {
                        LogMinerHelper.logWarn(streamingMetrics,
                                "LOB_ERASE for table '{}' is not known to the connector, skipped.", tableId);
                        continue;
                    }
                    transactionalBuffer.registerLobEraseOperation(operationCode, txId, scn, tableId,
                            changeTime.toInstant(), rowId, rsId);
                    break;
                }
    ...
    private Table getTableForDmlEvent(TableId tableId) throws SQLException {
        ...
        if (table == null) {
            if (connectorConfig.getTableFilters().dataCollectionFilter().isIncluded(tableId)) {
                ...
                LogMinerHelper.logWarn(streamingMetrics,
                        "DML for table '{}' that is not known to this connector, skipping.", tableId);
            }
        }
        return table;
    }
    ...
    private String getTableMetadataDdl(TableId tableId) throws SQLException {
        ...
        try (OracleConnection connection =
                new OracleConnection(connectorConfig.getJdbcConfig(), () -> getClass().getClassLoader())) {
            if (pdbName != null) {
